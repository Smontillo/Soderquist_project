...
static void corr (const int& nt, const double& dt, const double& staticErr, const int& nk, const mat& sdip, const cube& pdip, const cx_vec& bdip, const syst& s, const bath& b, const hidx& h, const int& xp) {
    ...
    cx_vec ftx = zeros<cx_vec>(nt); 
	cx_vec ftp = zeros<cx_vec>(nt); 

    d1.oprAct(rho1,sdip,pdip,bdip,rho0,'l',xp);
 
    FILE *log_tx = fopen("x.dat","w");
	FILE *log_tp = fopen("x2.dat","w");
    for (int it=0; it<nt; ++it) {
        double t = it*dt;
        ftx(it) = rho1(0,1,0)+rho1(1,0,0);
		ftp(it) = d1.Trace(sdip,pdip,bdip,rho1);
        printf ("In 1d-correlation: it=%d, nddo=%d, lddo=%d\n", it, d1.nddo, d1.lddo);
        fprintf(log_tx, "%16.6e%16.6e%16.6e\n", t/deom_fs2unit, real(ftx(it)), imag(ftx(it)));
		fprintf(log_tp, "%16.6e%16.6e%16.6e\n", t/deom_fs2unit, real(ftp(it)), imag(ftp(it)));
        d1.rk4 (rho1,t,dt);
		//d1.rk4 (rho2,t,dt);
    }
    fclose(log_tx);
    ...
    const cx_vec& fw = ifft(ftp)*nt*dt;
...
int main () {
    ...
    corr (nt, dt, staticErr, nk, sdip, pdip, bdip, s, b, h, xp);
