...
static void corr (const int& nt, const double& dt, const double& staticErr, const int& nk, const mat& sdip, const cube& pdip, const vec& bdip, const syst& s, const bath& b, const hidx& h, const int& xp) {
	...
	d1.rem(rho2,rho1,0.0);
 
    FILE *log_tx = fopen("x.dat","w");
	//FILE *log_tp = fopen("p.dat","w");
    for (int it=0; it<nt; ++it) {
        double t = it*dt;
        ftx(it) = rho2(0,1,0)+rho2(1,0,0);
		//ftp(it) = rho2(0,0,0)-rho2(1,1,0);
